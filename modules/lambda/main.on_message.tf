resource "aws_iam_role" "message_lambda_role" {
  name               = format("message_lambda_role_%s", var.random_name)
  assume_role_policy = data.aws_iam_policy_document.basic_lambda_role.json
}

resource "aws_lambda_function" "message" {
  filename         = "modules/lambda/archives/lambda_function_message.zip"
  function_name    = format("message_%s", var.random_name)
  description      = "Lambda API to handle WebSocket messages"
